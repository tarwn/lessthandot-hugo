...
<pre>CREATE TABLE #temp (contents varchar(max))

BULK INSERT #temp
   FROM 'C:resumesresume.txt';
GO

SELECT contents FROM #temp</pre>

OPENROWSET is another for reading the lot of the data

<pre>SELECT *
FROM OPENROWSET (BULK 'C:resumesresume.txt', SINGLE_CLOB) ResFile</pre>
